...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("𝒀𝒐𝒖'𝒓𝒆 𝑼𝒔𝒊𝒏𝒈 𝑶𝒏𝒆 𝑶𝒇 𝑴𝒚 𝑶𝒍𝒅 𝑴𝒆𝒔𝒔𝒂𝒈𝒆𝒔,𝑺𝒆𝒏𝒅 𝑻𝒉𝒆 𝑹𝒆𝒒𝒖𝒆𝒔𝒕 𝑨𝒈𝒂𝒊𝒏.", show_alert=True)
        return
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⛱️ 𝙷𝙾𝚆 𝚃𝙾 𝙳𝙾𝚆𝙽𝙻𝙾𝙰𝙳 🌟", url='https://t.me/how_2d_vk/5')
        ]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            return await query.answer('Pɪʀᴀᴄʏ Is Cʀɪᴍᴇ 🌚')
    ...
    if query.data.startswith("file"):
        ...
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('𝑵𝒐 𝑺𝒖𝒄𝒉 𝑭𝒊𝒍𝒆 𝑬𝒙𝒊𝒔𝒕 🥴')
        files = files_[0]
        title = files.file_name
        size = get_size(files.file_size)
        f_caption = files.caption
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"🎄 𝐑𝐞𝐪𝐮𝐞𝐬𝐭𝐞𝐝 𝐌𝐨𝐯𝐢𝐞 𝐍𝐚𝐦𝐞 : : <code>{search}</code>\n\n𝐈𝐟 𝐓𝐡𝐞 𝐌𝐨𝐯𝐢𝐞 𝐘𝐨𝐮'𝐫𝐞 𝐋𝐨𝐨𝐤𝐢𝐧𝐠 𝐅𝐨𝐫 𝐈𝐬 𝐍𝐨𝐭 𝐅𝐨𝐮𝐧𝐝 𝐓𝐡𝐞𝐧 𝐂𝐡𝐞𝐜𝐤 𝐓𝐡𝐞𝐬𝐞 𝐓𝐡𝐢𝐧𝐠𝐬👇 \n🗯️ 𝐓𝐲𝐩𝐞 𝐂𝐨𝐫𝐫𝐞𝐜𝐭 𝐒𝐩𝐞𝐥𝐥𝐢𝐧𝐠 𝐎𝐟 𝐓𝐡𝐞 𝐌𝐨𝐯𝐢𝐞.𝐄𝐱𝐚𝐦𝐩𝐥𝐞 (𝐌𝐚𝐬𝐭𝐞𝐫 𝟐𝟎𝟐𝟏). \n😪 𝐃𝐨𝐧'𝐭 𝐀𝐬𝐤 𝐔𝐧-𝐫𝐞𝐥𝐞𝐚𝐬𝐞𝐝 𝐌𝐨𝐯𝐢𝐞𝐬 𝐀𝐧𝐝 𝐀𝐥𝐬𝐨 𝐏𝐫𝐞𝐝𝐯𝐝 𝐅𝐢𝐥𝐞𝐬 𝐖𝐨𝐧'𝐭 𝐂𝐨𝐦𝐞 𝐇𝐞𝐫𝐞. \n\nSʜᴀʀᴇ Aɴᴅ Sᴜᴘᴘᴏʀᴛ 😀 \n\n\n𝚃𝚑𝚒𝚜 𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚆𝚒𝚕𝚕 𝙱𝚎 𝙳𝚎𝚕𝚎𝚝𝚎𝚍 𝚒𝚗 5 𝚖𝚒𝚗𝚞𝚝𝚎𝚜 🫰"
